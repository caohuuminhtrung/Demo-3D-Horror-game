...
public class QualitySettingsMenu : MonoBehaviour
{
	...
	[SerializeField] private Toggle fullscreenToggle; // Full Sreen off/on
	...
	private string[] shadowQualityList, shadowResolutionList, qualityList;
	...
	{	
        ...
        foreach (Resolution res in resolutionsList)
        {
            Debug.Log(res);
        }
        ...
        Debug.Log("Shadow quality:");
        foreach (string shadowQuality in shadowQualityList)
        {
            Debug.Log(shadowQuality);
        }

		shadowResolutionList = System.Enum.GetNames(typeof(ShadowResolution));
        Debug.Log("Shadow resolution:");

        foreach (string shadowResolution in shadowResolutionList)
        {
	...
	private void BuildMenu()
	{
		...
		{
			Dropdown.OptionData option = new Dropdown.OptionData();
			option.text = shadowQualityList[i];
		...
		{
			Dropdown.OptionData option = new Dropdown.OptionData();
			option.text = shadowResolutionList[i];
	...
	private void AnisoFiltering(int value)
	{
		if(value > 0)
		{
	...
	private string TextureResolutionNames(int value)
	{
		if(value == 0)
		{
			return "VeryHigh";
